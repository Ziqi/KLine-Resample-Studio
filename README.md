...
- **输入来源 (Input)**: 从 `1-KLine-Extract` 获取的一分钟级别 `.csv` 文件。
- **输出产物 (Output)**: 压实重组后的五分钟级别 `.csv` 文件。
  - **文件命名规则**: `[股票简称]_[代码]_5m_[开始日期]_to_[结束日期].csv` (自动替换 `1m`)。
  - **核心字段列表**: 严格保留及对齐 `timestamps`, `open`, `high`, `low`, `close`, `volume`, `amount` 格式。
- **架构设计缘由 (Why Resample?)**: 大规模的序列语言模型（尤其是 Kronos）在预训练时并不全盘需要 1m 的高噪信息，5 分钟能够大幅削弱高频杂波并保留趋势向量。在此过程中，硬编码的 Pandas 聚合策略精确对齐了中国 A 股（09:30 - 15:00）的物理交易时段分布规律。
